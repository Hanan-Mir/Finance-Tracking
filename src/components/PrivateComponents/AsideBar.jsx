...
function AsideBar() {
  ...
  return (
    <aside className="fixed shadow-box w-60 h-screen flex md:flex-col items-center dark:bg-slate-300 ">
      <div className="h-full flex md:flex-col md:justify-between md:gap-5">
        <div className="flex md:flex-col md:items-center gap-2 mt-4">
          ...
          Welcome, {session.user.user_metadata?.name?.toUpperCase() || session.user.user_metadata.email.slice(0,session.user.user_metadata.email.indexOf('@'))}
            ...
            <div className="flex md:items-end md:justify-between gap-5">
              <img
              ...
              <img
                ...
                alt=""
                className="w-12 h-12 p-2 hover:bg-gray-200 "
                onClick={() => signOut()}
              />
            </div>
          </div>
